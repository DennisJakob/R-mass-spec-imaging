...
# file name
file <- "tmp/20190523_MS37_MetaboliteMix_Spots_60_900_SDHB_pos_A25_75um_88x70" 
...
# create fragmentation matrix
mz_range <- matrix(1:(range*floor(length(mz)/range)),
                   nrow = range,
                   ncol = floor(length(mz)/range))
last_mz <- seq(from = floor(length(mz)/range)*range+1,
               to = length(mz),
               by = 1)

# create mz_range matrix
mz_values <- matrix(ncol = ncol(mz_range),
                    nrow = range)
for (i in 1:ncol(mz_range)) {
  mz_values[,i] <- mz[mz_range[,i]]
}
last_mz_values <- mz[last_mz]
